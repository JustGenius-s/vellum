...
`WorkspacePluginManifest` 统一描述功能 ID、导航标签、图标、面板或页面位置、懒加载组件、命令工厂和类型化依赖能力。插件运行时为每个视图与命令建立 capability scope，并支持插件私有 service。Files、Search、Outline、Tasks、Packages、Settings 都通过同一注册表进入侧栏、主页面和命令注册流程。Session 字段和既有命令 ID 未改变。
...
旧工作区组件使用 `useWorkspaceSelector`，插件 scope 内组件使用 `usePluginStore`。两类 selector 都具有结果缓存和相等比较；WorkspaceController 虽然仍广播状态版本，但未选中变化字段的组件不会重渲染。
